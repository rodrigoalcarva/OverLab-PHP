            ...
            <div class="containerStore">
                <div class='headerStore'>
                    <div class='optionStore'>
                        <a href='storeStores.php' style='text-align:left;color:#666666;'><p style="padding-top: 33%;font-size: 17px;margin-left: 25%;text-decoration:underline;">Lojas</p></a>
                    </div>
                ...
                <div id='categories'>
                    ...
                    <div id='d'>
                        <?php
                            $thestoreQuery = "select * from stores where plano='Premium'";
                            $getStores = mysqli_query($conn, $thestoreQuery);

                            if(!$getStores)
                                die("Error, select query failed:" . $thestoreQuery);

                            if(mysqli_num_rows($getStores)>0) {
                                while  ($row = mysqli_fetch_array($getStores)) {
                                        $idStore = $row[0];
                                        echo "<div class='theBrand'>
                                                    <a style='width:100%;height:100%;' href='theStore.php?name=$idStore'>
                                                        <div class='brandImage'>
                                                            <img src='storeImages/$idStore.jpg'>
                                                        </div>
                                                        <div class='brandText'>
                                                            <p style='margin: 0;font-size: 17px;color: black;padding-top: 1%;text-align:left;'>".$row[0]."</p>
                                                            <p style='font-size:12px;text-align:left;color:#999;'>".$row[4]."</p>
                                                        </div>
                                                    </a>
                                                </div>
                                               ";
                            ...
                            $thestoreQuery1 = "select * FROM stores where plano='Free'";
                            $getStores1 = mysqli_query($conn, $thestoreQuery1);

                            if(!$getStores1)
                                die("Error, select query failed:" . $thestoreQuery1);

                            if(mysqli_num_rows($getStores1)>0) {
                                while  ($row1 = mysqli_fetch_array($getStores1)) {
                                        $idStore = $row1[0];
                                        echo "<div class='theBrand'>
                                                    <a style='width:100%;height:100%;' href='theStore.php?name=$idStore'>
                                                        <div class='brandImage'>
                                                        ...
                                                        <div class='brandText'>
                                                            <p style='margin: 0;font-size: 17px;color: black;padding-top: 1%;text-align:left;'>".$row1[0]."</p>
                                                            <p style='font-size:12px;text-align:left;color:#999;'>".$row1[4]."</p>
                                                        </div>
                                                    </a>
                                                </div>
                                               ";
